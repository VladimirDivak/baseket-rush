...
var ASM_CONSTS = {
  8705512: () => { Module['emscripten_get_now_backup'] = performance.now; },  
 8705567: ($0) => { performance.now = function() { return $0; }; },  
 8705615: ($0) => { performance.now = function() { return $0; }; },  
 8705663: () => { performance.now = Module['emscripten_get_now_backup']; }
};
...
// end include: preamble.js
  ...
  function _Clear() {
      try {
        window.protectedLocalStorage.clear();
      } catch (error) {
        console.error(error);
      }
    }

  async function _GetItem(key, secretKey) {
      try {
        var keyStr = UTF8ToString(key);
        var secretKeyStr = UTF8ToString(secretKey);
        ...
        console.log(`jslib :: try get value ${keyStr}...`);
  
        var data = await window.protectedLocalStorage.getItemAsync(
          keyStr,
          secretKeyStr
        );
  
        if (data == null) {
          console.warn(`jslib :: ${keyStr} not found`);
          result.args = [keyStr, null];
        } else {
          result.args = [keyStr, data];
  
          console.log(`jslib :: send ${keyStr} to Unity`);
  ...
  async function _RemoveItem(key, secretKey) {
      try {
        var keyStr = UTF8ToString(key);
        var secretKeyStr = UTF8ToString(secretKey);
  
        await window.protectedLocalStorage.removeItemAsync(keyStr, secretKeyStr);
      } catch (error) {
        console.error(error);
      }
    }

  async function _SetItem(key, value, secretKey) {
      try {
        var keyStr = UTF8ToString(key);
        var valueStr = UTF8ToString(value);
        var secretKeyStr = UTF8ToString(secretKey);
  
        await window.protectedLocalStorage.setItemAsync(
          keyStr,
          valueStr,
          secretKeyStr
        );
